...
void setup() {
  ...
  if (dataFile) {
    // In der CSV-Datei speichern wir u.a. den kontinuierlichen Yaw sowie Pitch und Roll
...
void loop() {
  ...
  if (millis() - lastPrintTime >= 50) {
    ...
    if (!reset_done && elapsedMillis >= reset_time) {
      ...
      if (bno08x.getSensorEvent(&sensorValue)) {
        if (sensorValue.sensorId == SH2_ARVR_STABILIZED_RV) {
          ...
          // Setze die Initialwerte für Yaw (für kontinuierliches Unwrapping),
          // sowie für Pitch und Roll (Offset-Kalibrierung)
          last_raw_yaw = ypr.yaw;
          continuous_yaw = 0; // alternativ auch: continuous_yaw = ypr.yaw;
    ...
    if (GPS.fix) {
      snprintf(dateBuffer, sizeof(dateBuffer), "%02d/%02d/%04d", GPS.day, GPS.month, GPS.year + 2000);
    } else {
    ...
    if (bno08x.getSensorEvent(&sensorValue)) {
      if (sensorValue.sensorId == SH2_ARVR_STABILIZED_RV) {
        ...
        // Für Pitch und Roll erfolgt nun die Offset-Korrektur:
    ...
    // Erstellen der CSV-Zeile; nun wird der kontinuierliche Yaw sowie korrigierter Pitch und Roll ausgegeben
    char csvLine[200];
