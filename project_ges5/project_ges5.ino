  ...
  if (millis() - lastPrintTime >= 50) {
    ...
    if (bno08x.getSensorEvent(&sensorValue)) {
      if (sensorValue.sensorId == SH2_ARVR_STABILIZED_RV) {
        ...
        yaw = String(ypr.yaw);
        pitch = String(ypr.pitch);
        roll = String(ypr.roll);
      }
    }

    // CSV-Zeile erstellen
    String csvLine = String(elapsedSeconds) + "," + date + "," + time + "," + longitude + "," + latitude + "," + altitude + "," +
                     temp + "," + humid + "," + pressure + "," + yaw + "," + pitch + "," + roll;
