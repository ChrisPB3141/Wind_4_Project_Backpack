...
void setup() {
  ...
  if (dataFile) {
    // Spalte "elapsed_milliseconds" statt "elapsed_seconds"
    dataFile.println("elapsed_milliseconds,date,time,longitude,latitude,altitude,temp,humid,pressure,yaw,pitch,roll");
    dataFile.close();
  } else {
  ...
  if (millis() - lastPrintTime >= 50) {
    ...
    if (!reset_done && elapsedMillis >= reset_time) {
      ...
      if (bno08x.getSensorEvent(&sensorValue)) {
        if (sensorValue.sensorId == SH2_ARVR_STABILIZED_RV) {
          ...
          // Setze Offsets so, dass:
          // corrected_yaw = -(ypr.yaw - yaw_offset) = 0  --> yaw_offset = ypr.yaw
          // corrected_pitch = ypr.pitch - pitch_offset = 0
          // corrected_roll = ypr.roll - roll_offset = 0
          yaw_offset   = ypr.yaw;
    ...
    if (bno08x.getSensorEvent(&sensorValue)) {
      if (sensorValue.sensorId == SH2_ARVR_STABILIZED_RV) {
        ...
        // Für yaw: Invertiere den Unterschied, sodass eine gegen den Uhrzeigersinn verlaufende Drehung positive Werte liefert.
        float corrected_yaw   = -(ypr.yaw - yaw_offset);
        float corrected_pitch = -(ypr.pitch - pitch_offset);
        float corrected_roll  = ypr.roll - roll_offset;

        // Normalisiere den korrigierten Yaw in den Bereich [-180, 180]
        if (corrected_yaw > 180.0) corrected_yaw -= 360.0;
        if (corrected_yaw < -180.0) corrected_yaw += 360.0;

        // Optional: Auch Pitch und Roll normalisieren, falls nötig
    ...
    // Erstellen der CSV-Zeile; nun mit elapsedMillis in Millisekunden
    char csvLine[200];
